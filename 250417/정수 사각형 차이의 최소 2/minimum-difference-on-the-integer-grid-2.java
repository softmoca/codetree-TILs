...
public class Main {
    ...
    public static int solve(int lowerBound) {
        ...
        // 탐색하는 위치의 위에 값과 좌측 값 중에 작은 값과 해당 위치의 숫자 중에 최댓값을 구해줍니다.
        for (int i = 1; i < n; i++)
            for (int j = 1; j < n; j++)
                dp[i][j] = Math.min(
                        Math.max(dp[i - 1][j], num[i][j]), Math.max(dp[i][j - 1], num[i][j])
...
//                );
    ...
    public static void main(String[] args) {
        ...
        // 최솟값을 lowerBound라고 가정했을 때 lowerBound 이상의 수들만 사용하여
        // 이동한다는 조건하에서최댓값을 최소로 만드는 DP 문제를 풀어줍니다.
        for (int lowerBound = 1; lowerBound <= MAX_R; lowerBound++) {
            int upperBound = solve(lowerBound);

            // 다 진행했음에도 여전히 INT_MAX라면 그러한 이동이 불가능하다는 뜻이므로 패스합니다.
