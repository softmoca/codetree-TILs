...
public class Main {
    ...
    public static ArrayList<int[]> marbles = new ArrayList<>();
    ...
    // 그 상태를 반환합니다.
    public static int[] Move(int[] marble) {
        int x = marble[0];
        int y = marble[1];
        int dir = marble[2];
        ...
        // 설정해놨기 때문에 간단하게 처리가 가능합니다.
            ...
            return new int[]{x, y, 3 - dir};
    }
    
    // 구슬을 전부 한 번씩 움직여 봅니다.
    public static void moveAll() {
        for(int i = 0; i < marbles.size(); i++) {
            int[] nextMarble = Move(marbles.get(i));
    ...
    public static boolean duplicateMarbleExist(int targetIdx) {
        int targetX = marbles.get(targetIdx)[0];
        int targetY = marbles.get(targetIdx)[1];
    ...
    public static void removeDuplicateMarbles() {
        ArrayList<int[]> tempVector = new ArrayList<>();
        
        // Step2-1 : 각 구슬의 위치에 count를 증가시킵니다.
        for(int i = 0; i < marbles.size(); i++) {
            int x = marbles.get(i)[0];
            int y = marbles.get(i)[1];
            marbleCnt[x][y]++;
        }
    ...
    public static void main(String[] args) {
        ...
        while(t-- > 0) {
            ...
            for(int i = 1; i <= m; i++) {
                ...
                marbles.add(new int[]{x, y, mapper[d]});
            }
